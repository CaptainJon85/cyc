...
const AppBar = ({ active = "Home", role = "school", user = "SL", go }) => {
  const navs = {
    school: [["Home", "home"], ["Calculate", "calc"], ["Report", "report"], ["Plan", "plan"]],
    group: [["Overview", "group"], ["Settings", "groupsetting"], ["Reports", "group"]],
    kbt: [["Dashboard", "kbtanalytics"], ["Users", "kbtusers"], ["Content", "kbtcontent"]],
    pnz: [["Factors", "pnzfactors"], ["Ranges", "pnzbench"], ["Methodology", "pnzmethod"]],
  }[role];
  return (
    <div className="appbar">
      <div className="applogo"><span className="nav__mark">C</span> Count Your Carbon</div>
      <div className="appbar__nav">{navs.map(([n, id]) => <a key={n} className={cx(n === active && "on")} style={{ cursor: go && id ? "pointer" : "default" }} onClick={() => go && id && go(id)}>{n}</a>)}</div>
      <Spacer />
