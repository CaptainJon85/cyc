      ...
      return (
        <GRow g={0} className="row" style={{ minHeight: 600 }}>
          ...
          <div className="rail">
            ...
            {[["Overview", "group"], ["Settings", "groupsetting"], ["Reports", null], ["Engagement", null], ["Group targets", null]].map(([r, id], i) => (
              <div key={r} className={gcx("rail__item", i === 0 && "rail__item--on")} style={{ cursor: id ? "pointer" : "default", opacity: id ? 1 : 0.4, justifyContent: "space-between" }} onClick={() => id && go(id)}>
                <span>{r}</span>{!id && <span className="xs faint mono">soon</span>}
              </div>
            ))}
            <div style={{ height: 18 }} />
            <div className="rail__head">Admin</div>
            {["Associations", "Members"].map(r => <div key={r} className="rail__item" style={{ opacity: 0.4, justifyContent: "space-between" }}><span>{r}</span><span className="xs faint mono">soon</span></div>)}
          </div>
