...
/* which rail labels map to a built screen, per role */
const RAIL_MAP = {
  kbt: { "Dashboard": "kbtanalytics", "Users & settings": "kbtusers", "Content": "kbtcontent" },
  pnz: { "Emissions factors": "pnzfactors", "Benchmarks": "pnzbench", "Validation ranges": "pnzbench", "Methodology": "pnzmethod", "Sandbox": "pnzmethod" },
};
const APPBAR_ACTIVE = {
  kbt: { "Dashboard": "Dashboard", "Users & settings": "Users", "Content": "Content" },
  pnz: { "Emissions factors": "Factors", "Benchmarks": "Ranges", "Validation ranges": "Ranges", "Methodology": "Methodology" },
};

/* generic admin shell with left rail */
const AdminShell = ({ railTitle, items, active, role, user, children, go }) => {
  const map = RAIL_MAP[role] || {};
  return (
    <div className="row" style={{ minHeight: 600 }}>
      <div className="rail">
        ...
        {items.map(it => {
          ...
          return (
            <div key={it}
              className={acx("rail__item", it === active && "rail__item--on", !target && "rail__item--dim")}
              style={{ justifyContent: "space-between", cursor: target ? "pointer" : "default", opacity: target ? 1 : 0.4 }}
              onClick={() => target && go(target)}>
              <span>{it}</span>{!target && <span className="xs faint mono">soon</span>}
            </div>
          );
        })}
      </div>
      <div className="f1" style={{ minWidth: 0 }}>
        <AAppBar role={role} active={(APPBAR_ACTIVE[role] || {})[active] || ""} user={user} go={go} />
